...
import { parseStudentCSV, generateClassTemplateCSV, downloadCSV } from '../utils/csvHelper';
import { Upload, FileText, CheckCircle2, AlertCircle, X, Download, Users, BookOpen, Sparkles } from 'lucide-react';
      ...
      <div className="w-full max-w-2xl max-h-[90vh] rounded-3xl bg-slate-900 border border-slate-800 p-6 shadow-2xl flex flex-col space-y-4 text-white">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-400">Muat turun templat rasmi, isi maklumat kelas, dan muat naik semula</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <h4 className="text-sm font-bold text-white mb-1">
                Tarik & Lepaskan fail CSV kelas di sini
              </h4>
              <p className="text-xs text-slate-400 mb-4">
                atau klik untuk memilih fail daripada komputer anda
              </p>

              <label className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold cursor-pointer shadow-lg shadow-indigo-600/30 transition-all flex items-center gap-2">
                <Upload className="w-4 h-4" />
                <span>Pilih Fail CSV Kelas</span>
                <input
                  type="file"
                  accept=".csv,text/csv"
          ...
          <div className="space-y-3 flex-1 overflow-hidden flex flex-col">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-emerald-500/10 border border-emerald-500/30 p-3 rounded-xl text-emerald-300 text-xs gap-2">
              <div className="flex items-center gap-2">
                ...
                <span>
                  <strong>{parsedStudents.length} pelajar</strong> berjaya dibaca daripada fail <strong>{fileName}</strong>
                </span>
              </div>
              <button
                onClick={() => {
            ...
            <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-800">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-lg shadow-indigo-600/30 transition-all cursor-pointer flex items-center gap-1.5"
              >
                <CheckCircle2 className="w-4 h-4" />
                <span>Simpan {parsedStudents.length} Pelajar ke Master Kelas</span>
              </button>
            </div>
          </div>
        )}
